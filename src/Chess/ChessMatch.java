...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if(movedPiece instanceof Pawn) {
			if((movedPiece.getColor() == Color.WHITE && target.getRow() == 0) ||
				...
				promoted = replacePromotedPiece("Q");//por padrão começo trocando pela rainha
			}
		}
	...
	//método auxiliar para instanciar a nova peça
	...
	private Piece makeMove(Position source, Position target) {
		ChessPiece p = (ChessPiece) board.removePiece(source);// tira a peça da origem
		p.increaseMoveCount();
		...
		if(p instanceof Pawn) {
			//se meu peão andou na diagonal e não capturou peça é porque foi en passant
